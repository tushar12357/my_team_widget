      ...
      <div className="w-full max-w-sm sm:max-w-md md:max-w-6xl mx-auto">
        ...
        {!showSequence ? (
          <div className="max-w-sm sm:max-w-md mx-auto">
            <div 
              className="relative bg-gradient-to-br from-white via-orange-50 to-red-50 shadow-2xl p-6 sm:p-10 transform transition-all duration-700 hover:scale-100 sm:hover:scale-105"
              style={{
            ...
            >
              ...
              <div className="flex flex-col items-center text-center space-y-6 relative z-10">
                ...
                <div className="relative">
                  ...
                  >
                    <MessageCircle className="w-10 h-10 sm:w-12 sm:h-12 text-white relative z-10" strokeWidth={2.5} />
                  </div>
                </div>
                ...
                >
                  <Zap className="w-5 h-5 sm:w-6 sm:h-6 relative z-10" strokeWidth={2.5} />
                  <span className="relative z-10 text-base sm:text-lg tracking-wide">Watch Demo</span>
                  <Send className="w-5 h-5 sm:w-6 sm:h-6 relative z-10" strokeWidth={2.5} />
                </button>
          ...
          <div className="bg-gradient-to-br from-slate-900 via-gray-900 to-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-gray-800">
            ...
            <div className="bg-gradient-to-r from-orange-600 to-red-600 p-4 sm:p-6 relative">
              <button
                onClick={closeSequence}
                className="absolute top-3 sm:top-4 right-3 sm:right-4 bg-white bg-opacity-20 hover:bg-opacity-30 text-white rounded-full p-1.5 sm:p-2 transition-all"
              >
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="bg-white bg-opacity-20 p-3 sm:p-4 rounded-2xl">
                  <Activity className="w-8 h-8 sm:w-10 sm:h-10 text-white" strokeWidth={2.5} />
                </div>
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-white">Live Sequence Monitor</h2>
                  <p className="text-orange-100 text-xs sm:text-sm">Maya's Automated Follow-Up System</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 sm:p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
                <div className="bg-gradient-to-br from-orange-900 to-orange-800 rounded-xl p-3 sm:p-4 border border-orange-700">
                  <PhoneCall className="w-6 h-6 sm:w-8 sm:h-8 text-orange-300 mb-2" />
                  <div className="text-2xl sm:text-3xl font-bold text-white">{sequenceStats.calls}</div>
                  <div className="text-orange-300 text-xs sm:text-sm">AI Calls</div>
                </div>
                
                <div className="bg-gradient-to-br from-green-900 to-green-800 rounded-xl p-3 sm:p-4 border border-green-700">
                  <MessageCircle className="w-6 h-6 sm:w-8 sm:h-8 text-green-300 mb-2" />
                  <div className="text-2xl sm:text-3xl font-bold text-white">{sequenceStats.whatsapp}</div>
                  <div className="text-green-300 text-xs sm:text-sm">WhatsApp</div>
                </div>
                
                <div className="bg-gradient-to-br from-blue-900 to-blue-800 rounded-xl p-3 sm:p-4 border border-blue-700">
                  <Phone className="w-6 h-6 sm:w-8 sm:h-8 text-blue-300 mb-2" />
                  <div className="text-2xl sm:text-3xl font-bold text-white">{sequenceStats.sms}</div>
                  <div className="text-blue-300 text-xs sm:text-sm">SMS</div>
                </div>
                
                <div className="bg-gradient-to-br from-purple-900 to-purple-800 rounded-xl p-3 sm:p-4 border border-purple-700">
                  <Mail className="w-6 h-6 sm:w-8 sm:h-8 text-purple-300 mb-2" />
                  <div className="text-2xl sm:text-3xl font-bold text-white">{sequenceStats.emails}</div>
                  <div className="text-purple-300 text-xs sm:text-sm">Emails</div>
                </div>
              </div>
              ...
              <div className="bg-gray-800 rounded-2xl p-4 sm:p-6 max-h-80 sm:max-h-[500px] overflow-y-auto">
                ...
                {sentMessages.length === 0 && (
                  <div className="text-center py-8 sm:py-12">
                    <Clock className="w-12 h-12 sm:w-16 sm:h-16 text-gray-600 mx-auto mb-4 animate-spin" />
                    <p className="text-gray-400 text-xs sm:text-sm">Initializing automated sequence...</p>
                  </div>
                )}
                ...
                <div className="space-y-3">
                  {sentMessages.map((msg, idx) => {
                    ...
                    return (
                      <div 
                        key={idx}
                        className="bg-gray-900 rounded-xl p-3 sm:p-4 border border-gray-700 animate-slideIn"
                      >
                        <div className="flex items-start gap-2 sm:gap-3">
                          <div className={`p-2 sm:p-3 rounded-xl ${msg.color} bg-opacity-20 border border-current`}>
                            <Icon className={`w-5 h-5 sm:w-6 sm:h-6 ${msg.color.replace('bg-', 'text-')}`} strokeWidth={2.5} />
                          </div>
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-gray-300 text-xs sm:text-sm leading-relaxed">{msg.message}</p>
                ...
                {currentTouchpoint >= touchpoints.length && (
                  <div className="mt-4 sm:mt-6 bg-gradient-to-br from-green-900 to-emerald-900 rounded-xl p-4 sm:p-6 border border-green-700">
                    <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                      <TrendingUp className="w-6 h-6 sm:w-8 sm:h-8 text-green-400" />
                      <div>
                        <h4 className="text-white font-bold text-base sm:text-lg">Sequence Complete!</h4>
                        <p className="text-green-300 text-xs sm:text-sm">All touchpoints executed successfully</p>
                      </div>
                    </div>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-3 sm:mt-4">
                      <div className="bg-gray-900 bg-opacity-50 rounded-lg p-3">
                        <div className="text-xl sm:text-2xl font-bold text-white">{touchpoints.length}</div>
                        <div className="text-green-300 text-xs">Total Touchpoints</div>
                      </div>
                      <div className="bg-gray-900 bg-opacity-50 rounded-lg p-3">
                        <div className="text-xl sm:text-2xl font-bold text-white">15 Days</div>
                    ...
                    <button
                      onClick={closeSequence}
                      className="mt-3 sm:mt-4 w-full bg-white text-green-700 py-2 sm:py-3 rounded-lg font-bold hover:bg-gray-100 transition-all"
                    >
                      Close & Return
                    </button>
                  </div>
